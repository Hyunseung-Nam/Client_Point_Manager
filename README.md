...
이 프로젝트는 실제 운영 환경에서 사용된 기능을 기반으로,
일반화된 구조와 로직으로 정리한 데스크톱 애플리케이션입니다.

> 본 레포에는 실제 고객 또는 사용자 데이터가 포함되지 않습니다.
---

## 주요 기능

- 사용자 등록 및 활동 A/B 횟수 입력·수정  
- 누적 기준 충족 시 자동 포인트 지급(예: 10회 1000포인트) 및 지급 로그 기록  
...
- **v1.3**
  - exe 실행 시 AppData 경로로 데이터/백업/로그 저장 경로 분리
  - JSON 로드 예외 처리 중복 제거 및 로직 정리

- **v1.2**
  - 지급 전·후 상태를 포함한 이력 기반 추적 구조 적용
  - 다중 선택 시 조건 미충족 사용자 알림 추가
  - 포인트 지급 버튼 중복 클릭 방지
  - 백업 기능 추가
  - 전화번호 010 시작/11자리 검증 및 기존 데이터 정리 함수(마이그레이션 함수) 추가(최초 1회)
  - 지급 조건 경계 버그 수정
  - 10회 미만 지급 제한 추가
